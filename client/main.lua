...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("baker-carradio:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("baker-carradio:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("baker-carradio:ChangeLoop",nameid,datasoundinfo.loop)
		else
			datasoundinfo.loop = not datasoundinfo.loop
		end
		if type(datasoundinfo.loop) ~= "table" then
			local loop = ('<b>KARIŞTIRICI:</b> '.. firstToUpper(tostring(datasoundinfo.loop)))
			SendNUIMessage({
				action = "changetextl",
				text = loop,
			})
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("baker-carradio:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		local volume = (('<b>SES:</b> '.. math.floor((vadi*100) - 0.1+1).."%"))
		SendNUIMessage({
            action = "changetextv",
            text = volume,
        })
		TriggerServerEvent("baker-carradio:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("baker-carradio:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("baker-carradio:AddVehicle",vehdata)
			end
		end
	else
...
if Config.ItemInVehicle then
	RegisterNetEvent("baker-carradio:ShowNui")
	AddEventHandler("baker-carradio:ShowNui", function()
		show()
	end)
end
...
function show(nomecenas)
    ...
    if shown and nome then
		...
		local volume = ('<b>SES:</b> '.. math.floor((datasoundinfo.volume*100) - 0.1+1).."%")
		if type(datasoundinfo.loop) ~= "table" then
			local loop = ('<b>KARIŞTIRICI:</b> '.. firstToUpper(tostring(datasoundinfo.loop)))
			SendNUIMessage({
				action = "changetextl",
    ...
    elseif nuiaberto then
		...
		QBCore.Functions.Notify("Sadece araç içinde kullanabilirsiniz.", "error")
	end
end
...
AddEventHandler("baker-carradio:AddVehicle", function(data)
...
AddEventHandler("baker-carradio:ChangeState", function(tipo, nome)
...
AddEventHandler("baker-carradio:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("baker-carradio:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("baker-carradio:SendData")
AddEventHandler("baker-carradio:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
